...
def train_deep_learning_model(single_phase_data):

    # Using Interactive session makes it the default sessions so we do not need to pass sess
    sess = tf.InteractiveSession()

    # Define placeholders for input data
    with tf.name_scope("Single_Phase_Input"):
        x = tf.placeholder(tf.float32, shape=[None, 784], name="x")
        y_ = tf.placeholder(tf.float32, [None, 10], name="y_")

    # change the Single Phase input data from a list of values to a 28 pixel X 28 pixel X 1 grayscale value cube
    #    which the Convolution NN can use.
    with tf.name_scope("Input_Reshape"):
        x_phase = tf.reshape(x, [-1, 28, 28, 1], name="x_phase")
        tf.summary.phase('input_img', x_phase, 5)

    # Define the Model

    # 1st Convolution layer
    with tf.name_scope('Conv1'):
        # 32 features for each 5X5 patch of the phase
        ...
        # Do convolution on phases, add bias and push through RELU activation
        conv1_wx_b = conv2d(x_phase, W_conv1,name="conv2d") + b_conv1
    ...
    with tf.name_scope("cross_entropy"):
        cross_entropy = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=y_conv, states=y_))
    ...
    for i in range(num_steps):
        batch = single_phase_data.train.next_batch(50)
    ...
    print("Test accuracy {0:.3f}%".format(accuracy.eval(feed_dict={
        x: single_phase_data.test.phases, y_: single_phase_data.test.states, keep_prob: 1.0})*100.0))